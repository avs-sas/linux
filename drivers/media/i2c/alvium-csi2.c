...
static int alvium_cmd_fw_get_lane_cnt(struct v4l2_subdev *sd, void *arg)
{
	struct alvium_dev *alvium = sd_to_alvium(sd);

	return alvium->h_sup_csi_lanes;
}
...
static long alvium_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case ALVIUM_FW_GET_LANE_CNT_I2C_R:
		return alvium_cmd_fw_get_lane_cnt(sd, arg);
