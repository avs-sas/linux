					...
					struct v4l2_subdev_state *state)
{
	struct v4l2_mbus_framefmt *fmt =
		v4l2_subdev_state_get_format(state, MAX96716_PAD_SOURCE);
				 ...
				 struct v4l2_subdev_state *state)
{
	...
	if (priv->pattern)
		ret = max96716_apply_patgen_timing(priv, state);
				   ...
				   u32 source_pad, u64 streams_mask)
{
	...
	ret = max96716_apply_patgen(priv, state);
	if (ret)
		goto err;
...
static int max96716_init_state(struct v4l2_subdev *sd,
			       struct v4l2_subdev_state *state)
{
...
static const struct v4l2_subdev_internal_ops max96716_internal_ops = {
	.init_state = max96716_init_state,
};
...
static void max96716_v4l2_notifier_unregister(struct max96716_priv *priv)
{
...
err_unreg_notif:
	max96716_v4l2_notifier_unregister(priv);
err_subdev_cleanup:
